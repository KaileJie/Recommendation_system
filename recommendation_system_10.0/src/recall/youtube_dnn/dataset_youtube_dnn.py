...
class YoutubeDNNDataset(Dataset):
    def __init__(self,
        ...
        if filter_item_ids is not None:
            self.valid_item_ids &= filter_item_ids  # ✅ 取交集
        ...
        self.num_std = self.users[self.num_cols].std(ddof=0).replace(0.0, 1.0).values.astype(np.float32)

        self.items["text"] = (
            self.items["title"].fillna("") + " " + self.items["content"].fillna("")
        )
        ...
        self.item_text_feats = {
            iid: tfidf_matrix[i].toarray()[0]
            for i, iid in enumerate(self.items["item_id"])
        }
        try:
        ...
        if not self.item_text_feats:
            ...
            self.item_text_feats = {iid: np.zeros(max_text_feat_dim, dtype=np.float32) for iid in self.item2idx}
        ...
        for chunk in iter_clean_behavior(
            ...
            chunk = chunk[chunk["item_id"].isin(self.valid_item_ids)]  # ✅ 过滤非法 item_id
            chunk = apply_time_weight(chunk,
                                      dt_col="dt",
        ...
        self.user2items = (
            self.interactions.groupby("user_id")["item_id"]
            .apply(set).to_dict()
        )
        ...
        for row in self.users.itertuples(index=False):
            ...
            num_vec = np.array([row.recency, row.frequency, row.actions_per_active_day_30d], dtype=np.float32)
    ...
    def _sample_negatives(self, user_id, num_samples):
        ...
        if not candidates:
            return []
        return np.random.choice(candidates, size=num_samples, replace=len(candidates) < num_samples)
    ...
    def __getitem__(self, idx):
        ...
        item_text = self.item_text_feats.get(item_id, np.zeros_like(next(iter(self.item_text_feats.values()))))
        ...
        return (
            ...
            torch.tensor(num_z, dtype=torch.float),
            torch.tensor(item_text, dtype=torch.float),
            torch.tensor(time_weight, dtype=torch.float)
        )
