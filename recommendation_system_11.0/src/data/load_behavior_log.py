...
# ----------- Config -----------
# ✅ 从统一模块导入 RFM_WEIGHTS 和 VALID_ACTIONS
from src.utils.weight_utils import RFM_WEIGHTS
...
def build_datetime(year_series, mmd_series, sec_series):
    ...
    try:
        ...
        full_dt = date_part + pd.to_timedelta(seconds, unit='s')

        # ✅ 新增：记录失败的行用于排查
        if full_dt.isna().any():
            failed = full_dt[full_dt.isna()]
            logger.warning(f"[build_datetime] Datetime parse failed for {len(failed)} rows. Example rows:")
            logger.warning(pd.DataFrame({
                'year': year[failed.index],
                'mmd': mmd_series[failed.index],
                'sec': sec_series[failed.index]
