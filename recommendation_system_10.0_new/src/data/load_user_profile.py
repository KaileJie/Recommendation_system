...
def load_user_profile_feature(filepath: str) -> pd.DataFrame:
    ...
    try:
        ...
        df['gender'] = df['gender'].apply(lambda x: x if x in [0, 1] else -1)

        # age_range: 0~8 为合法年龄段，-1=未知
        df['age_range'] = pd.to_numeric(df['age_range'], errors='coerce').fillna(-1).astype(int)
        df['age_range'] = df['age_range'].apply(lambda x: x if x in range(0, 9) else -1)
