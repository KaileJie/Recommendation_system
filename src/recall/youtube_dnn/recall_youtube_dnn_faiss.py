...
def build_and_run_recall():
    ...
    sequence_masks = []
    for uid in user_ids:
        seq, mask = dataset.get_user_sequence(uid)
        user_sequences.append(seq)
        sequence_masks.append(mask)
    
    user_sequence = torch.tensor(user_sequences, dtype=torch.long)
    sequence_mask = torch.tensor(sequence_masks, dtype=torch.long)
