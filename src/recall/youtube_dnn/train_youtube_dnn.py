...
class WeightedSoftmaxLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.loss_fn = nn.CrossEntropyLoss(reduction='none')  # 不自动求平均

    def forward(self, logits, sample_weights=None):
        """
        Args:
            logits: [batch_size, 2] 预测logits
            sample_weights: [batch_size] 样本权重，如果为None则使用均匀权重
        """
        batch_size = logits.size(0)
        labels = torch.zeros(batch_size, dtype=torch.long, device=logits.device)
        
        # 计算每个样本的loss
        losses = self.loss_fn(logits, labels)  # [batch_size]
        
        # 应用样本权重
        if sample_weights is not None:
            # 确保权重形状匹配
            if sample_weights.dim() == 0:
                sample_weights = sample_weights.unsqueeze(0)
            if sample_weights.size(0) != batch_size:
                sample_weights = sample_weights[:batch_size]
            
            # 加权平均
            weighted_loss = (losses * sample_weights).sum() / sample_weights.sum()
        else:
            # 均匀权重
...
def evaluate_recall_at_k(model, dataset, k=10):
    ...
    for user_id in dataset.interactions["user_id"].unique():
        ...
        # 获取用户历史序列
        user_seq, seq_mask = dataset.get_user_sequence(user_id)
        user_sequence = torch.tensor([user_seq])
        sequence_mask = torch.tensor([seq_mask])

        with torch.no_grad():
            user_emb = model.get_user_embedding(uidx, gender, age, city, cluster, num_feat, 
                                              user_sequence, sequence_mask)
...
def main():
    ...
    logger.info("🎯 Using weighted loss: time_weight * rfm_weight")
    ...
    criterion = WeightedSoftmaxLoss()  # ✅ 使用加权loss函数
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(train_loader):
            (
                ...
                time_weight, user_sequence, sequence_mask, rfm_weight
            ) = batch
