...
def main():
    ...
    # 按用户分组拆分训练/验证集，避免数据泄露
    gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
    train_idx, val_idx = next(gss.split(X, y, groups=df["user_id"]))
    
    X_train, X_val = X.iloc[train_idx], X.iloc[val_idx]
    y_train, y_val = y.iloc[train_idx], y.iloc[val_idx]
    train_df = df[["user_id", "item_id"]].iloc[train_idx]
    val_df = df[["user_id", "item_id"]].iloc[val_idx]
    
    logger.info(f"✅ Train/Val split by user: {len(train_idx)} train samples, {len(val_idx)} val samples")
    logger.info(f"✅ Train users: {df['user_id'].iloc[train_idx].nunique()}, Val users: {df['user_id'].iloc[val_idx].nunique()}")
